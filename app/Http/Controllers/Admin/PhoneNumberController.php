...
class PhoneNumberController extends AdminBaseController
{
    ...
    #[NoReturn] public function search(Request $request): void
    {
        ...
        $totalData = PhoneNumbers::where('server', '!=', 'turbo s')->count();
        ...
        if (empty($request->input('search.value'))) {
            $numbers = PhoneNumbers::where('server', '!=', 'turbo s')->offset($start)
                                   ->limit($limit)
                                   ->orderBy($order, $dir)
                                   ->get();
        } else {
            ...
            $numbers = PhoneNumbers::where('server', '!=', 'turbo s')->whereLike(['uid', 'number', 'price', 'status', 'user.first_name', 'user.last_name'], $search)
                                   ->offset($start)
                                   ->limit($limit)
                                   ->orderBy($order, $dir)
                                   ->get();

            $totalFiltered = PhoneNumbers::where('server', '!=', 'turbo s')->whereLike(['uid', 'number', 'price', 'status', 'user.first_name', 'user.last_name'], $search)->count();
    ...
    public function create(): Factory|View|Application
    {
        ...
        $servers = SendingServer::where('status', 1)->where('user_id', auth()->user()->id)->get();

        return view('admin.PhoneNumbers.create', compact('breadcrumbs', 'currencies', 'customers','servers'));
    }
    ...
    public function show(PhoneNumbers $number): \Illuminate\Contracts\View\View|Factory|RedirectResponse|Application
    {
        ...
        if (isset($capabilities) && is_array($capabilities) && count($capabilities) > 0) {
            return view('admin.PhoneNumbers.show', compact('breadcrumbs', 'number', 'customers', 'currencies', 'capabilities','servers'));
        }
