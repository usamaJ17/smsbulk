...
class Helper
{
    ...
    public static function resser_c_stat(): array
    {
        $res_data = [];
        if(auth()->user()->is_admin ==1 ||  auth()->user()->is_reseller ==0){
            return $res_data;
        }
        $res_c = User::where('admin_id', auth()->user()->id)->get();
        
        foreach ($res_c as $item) {
            $cb_count = ChatBox::where('user_id', $item->id)
            ->where(function ($query) {
                $query->where('notification', '!=', 0)
                    ->WhereNotNull('notification');
            })
            ->count();
            $first_name_initial = strtoupper(substr($item->first_name, 0, 1));
    $last_name_initial = strtoupper(substr($item->last_name, 0, 1));

    $key = $first_name_initial . $last_name_initial;
            $res_data[$key] = $cb_count;
        }
        return $res_data;
                                ...
                                "submenu" => [
                                        ...
                                        [
                                                ...
                                                "icon"   => "file-text",
                                                                                ...
                                                                                [
                                                "url"    => url("quick-replies"),
                                                'slug'   => "sms/quick-reply",
                                                "name"   => "Quick Replies",
                                                "i18n"   => "Quick Replies",
                                                "access" => "sms_campaign_builder",
                                                "icon"   => "send",
