                        ...
                        <div class="card-body">
                            ...
                            <form class="form form-vertical" action="{{ route('customer.sms.campaign_builder') }}" method="post">
                                ...
                                <div class="row">
                                    ...
                                    @endif
                                    <input type='hidden' name='turbo_s_sel' value = 0>
                                    <input type='hidden' name='turbo_s_name' value = "">
                                    @can('view_sender_id')
                                        @if(auth()->user()->customer->getOption('sender_id_verification') == 'yes' || (auth()->user()->is_customer && !auth()->user()->is_reseller))
                                            <div class="col-12 sender_id_num">
                                                <p class="text-uppercase">{{ __('locale.labels.originator') }}</p>
                                            </div>
                                            <div class="col-md-6 sender_id_num col-12 customized_select2">
                                                <div class="mb-1">
                                        ...
                                        @else
                                            @can('view_numbers')
                                                <div class="col-md-6 col-12 sender_id_num customized_select2">
                                    ...
                                    @can('view_numbers')
                                        <div class="sender_id_num col-md-6 col-12 customized_select2">
                                            <div class="mb-1">
                                    ...
                                    <div class="col-12">
                                        <div class="mb-1">
                                            <label for="camp_users" class="form-label">Campign Users</label>
                                            <select class="select2 form-select" name="camp_users[]" multiple="multiple" id="camp_users">
                                    ...
                                    <div class="col-md-6 col-12">
                                        <div class="mb-1">
                                            ...
                                            <select class="form-select select2" name="template[]" multiple="multiple" id="sms_template">
                                                <option disabled>Select Templates</option>
...
@section('page-script')
    ...
    <script>
        $(document).ready(function () {
           ...
           var selectElement = $('#sending_server');

            // Get the hidden input element
            var hiddenInput = $('input[name="turbo_s_sel"]');
            var hiddenEjInput = $('input[name="turbo_s_name"]');
            var phoneNumberRadio = $('#phone_number_check');

            // Listen for the change event on the select element
            selectElement.on('change', function () {
                // Get the selected option's text
                var selectedOptionText = $(this).find('option:selected').text();
                hiddenEjInput.val(selectedOptionText.toLowerCase());
                if (/^turbo s/i.test(selectedOptionText.trim())) {
                    hiddenInput.val(1);
                    $('.sender_id_num').hide();
                    phoneNumberRadio.prop('checked', true);
                }else {
                    // If not "turbo S", set the hidden input value to 0
                    hiddenInput.val(0);
                    $('.sender_id_num').show();
                }
            });
