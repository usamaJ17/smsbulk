                            ...
                            <form class="form form-vertical" action="{{ route('admin.phone-numbers.update',  $number->uid) }}" method="post">
                                ...
                                <div class="form-body">
                                    <div class="row">
                                        ...
                                        <div class="col-12">
                                            <div class="mb-1">
                                                <label for="server" class="form-label required">Sending Server</label>
                                                <select class="form-select select2" id="server" name="server">
                                                    <option value=null>-- Select Server --</option>
                                                    @foreach($servers as $server)
                                                        <option value="{{strtolower($server->name)}}" @if($number->server == strtolower($server->name)) selected @endif>{{$server->name}}</option>
                                                    @endforeach
                                                </select>
                                                @error('server')
                                                <p><small class="text-danger">{{ $message }}</small></p>
